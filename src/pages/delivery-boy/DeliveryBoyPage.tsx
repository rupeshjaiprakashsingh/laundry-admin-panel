      ...
      <Dialog open={!!confirmDialog} onClose={() => setConfirmDialog(null)} maxWidth="xs" fullWidth>
        ...
        <DialogActions sx={{ px: 2, pb: 2 }}>
          ...
          <Button
            variant="contained"
            disabled={isLoading}
            onClick={() => { if (confirmDialog) { onAction(pickup.id, confirmDialog.status); setConfirmDialog(null); } }}
            sx={{
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
            {confirmDialog?.status === 'Delivered'
              ...
              : `Mark this delivery as failed? The order will return to "Ready For Delivery" status.`}
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 2, pb: 2 }}>
          <Button onClick={() => setConfirmDialog(null)} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 600 }}>No, Go Back</Button>
          <Button
            variant="contained"
            disabled={isLoading}
            onClick={() => { if (confirmDialog) { onAction(delivery.id, confirmDialog.status); setConfirmDialog(null); } }}
            sx={{
              borderRadius: 2, textTransform: 'none', fontWeight: 700,
...
const DeliveryBoyPage: React.FC = () => {
  ...
  const pickupMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      updateMyPickupStatus(id, status),
    onSuccess: () => {
  ...
  const deliveryMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      updateMyDeliveryStatus(id, status),
    onSuccess: () => {
            ...
            {pickupsLoading ? (
              ...
              pickups.map((pickup) => (
                <PickupCard
                  key={pickup.id}
                  pickup={pickup}
                  onAction={(id, status) => pickupMutation.mutate({ id, status })}
            ...
            {deliveriesLoading ? (
              ...
              deliveries.map((delivery) => (
                <DeliveryCard
                  key={delivery.id}
                  delivery={delivery}
                  onAction={(id, status) => deliveryMutation.mutate({ id, status })}
